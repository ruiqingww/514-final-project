...
void writeFloatData(
  ...
  if (length >= sizeof(float)) {
    ...
    memcpy(&curData, pData, sizeof(float));

    // Now, convertedValue holds the float representation of the first 4 bytes pointed to by pd
}
else{
    Serial.println("Error when converting raw received data!");
}
}
